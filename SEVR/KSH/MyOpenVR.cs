...
namespace VRage.OpenVRWrapper
{
    ...
    public class MyOpenVR
    {
        ...
        private static VRageRender.MyViewport m_viewport;
        ...
        public MyOpenVR()
        {
            if (true)
            {
                ...
                m_vrSystem = OpenVR.Init(ref error, EVRApplicationType.VRApplication_Scene);
                if (error == EVRInitError.None)
                {
                    ...
                    m_vrSystem.GetProjectionRaw(EVREye.Eye_Left, ref l_left, ref l_right, ref l_top, ref l_bottom);

                    float r_left = 0.0f, r_right = 0.0f, r_top = 0.0f, r_bottom = 0.0f;
                    m_vrSystem.GetProjectionRaw(EVREye.Eye_Right, ref r_left, ref r_right, ref r_top, ref r_bottom);

                    float tanHalfFov = new List<float>() { -l_left, l_right, -r_left, r_right }.Max();

                    m_fov = (float)((2.0f * Math.Atan(tanHalfFov) * 360) / (3.14159265358979323846 * 2));

                    OpenVR.GetGenericInterface(OpenVR.IVRCompositor_Version, ref error);
                    if (error == EVRInitError.None)
                    {
                        m_vrCompositor = OpenVR.Compositor;
        ...
        private static Matrix PoseToViewMatrix(HmdMatrix44_t pose)
        {
            ...
            mat.M11 = pose.m0;// 0.7737882f;//;
            mat.M22 = pose.m5; //0.694595751f; 

            mat.M33 = 0f;//pose.m10;
            mat.M34 = pose.m14;

            mat.M43 = -pose.m11;//-0.05000017f; //pose.m11;
        ...
        public static VRageRender.MyViewport ViewportForEye(EVREye eye)
        {
            m_viewport.OffsetX = eye == EVREye.Eye_Left ? 0 : 0.5f;
            return m_viewport;
        }

        public static Matrix GetEyeProjection(EVREye eye, float near, float far) 
            => PoseToViewMatrix(m_vrSystem.GetProjectionMatrix(eye, near, far));

        private static Matrix MatrixFromOVRMatrix44(HmdMatrix44_t pose)
        {
            Matrix mat = Matrix.Identity;
            mat.M11 = pose.m0;
            mat.M12 = pose.m4;
        ...
        private static bool InitOverlays()
        {
            ...
            OpenVR.GetGenericInterface(OpenVR.IVROverlay_Version, ref iError);
        ...
        {
            hmdMatrix.m0 = mat.M11;
            hmdMatrix.m4 = mat.M12;
            hmdMatrix.m8 = mat.M13;

            hmdMatrix.m1 = mat.M21;
            hmdMatrix.m5 = mat.M22;
            hmdMatrix.m9 = mat.M23;

            hmdMatrix.m2 = mat.M31;
            hmdMatrix.m6 = mat.M32;
            hmdMatrix.m10 = mat.M33;

            hmdMatrix.m3 = mat.M41;
            hmdMatrix.m7 = mat.M42;
            hmdMatrix.m11 = mat.M43;
        }
        ...
        {
            if (m_cvrOverlay.SetOverlayInputMethod(handle, VROverlayInputMethod.Mouse) == EVROverlayError.None)
            {
        ...
        public static void FrameDone()
        {
            ...
            //if (m_vrCompositor != null) m_vrCompositor.PostPresentHandoff();
        }
        ...
        public void DisplayEye(IntPtr nativePointer, EVREye eye)
        {
            ...
            tboud.uMin = 0;
            tboud.uMax = 1;
            var error = m_vrCompositor.Submit(eye, ref tex, ref tboud, EVRSubmitFlags.Submit_Default);
        }
        ...
        protected static void SetIPD(float ipd)
        {
            ...
            uint intWidth = 0, intHeight = 0;
            m_vrSystem.GetRecommendedRenderTargetSize(ref intWidth, ref intHeight);
            float width = (float)intWidth, height = (float)intHeight;
            ...
            float TOTAL_CUT = (width - height) / width;
            float IPD_MAX = (width - height) / width;
        ...
        public static Vector2[] GetStencilMask()
        {
            HiddenAreaMesh_t leftMesh = m_vrSystem.GetHiddenAreaMesh(EVREye.Eye_Left, EHiddenAreaMeshType.k_eHiddenAreaMesh_Standard);
            HiddenAreaMesh_t rightMesh = m_vrSystem.GetHiddenAreaMesh(EVREye.Eye_Right, EHiddenAreaMeshType.k_eHiddenAreaMesh_Standard);
